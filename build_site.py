...
def load(path: Path = COMBINED) -> dict:
...
<meta name="exposome-record-count" content="%%N%%">
<title>Exposome / EWAS Literature Collection</title>
<meta name="description" content="Searchable all-age inventory of %%N%% exposome / EWAS manuscripts, %%EHR%% EHR-based, spanning %%YMIN%%-%%YMAX%%. Model provenance: %%MODEL%%.">
...
<header class="border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/60 backdrop-blur sticky top-0 z-10">
  <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between gap-4">
    <div>
      <h1 class="text-lg sm:text-xl font-bold tracking-tight">Exposome / EWAS Literature Collection</h1>
      <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Model provenance: %%MODEL%% · %%YMIN%%–%%YMAX%%</p>
    </div>
...
<footer class="border-t border-slate-200 dark:border-slate-800 py-6 text-center text-xs text-slate-500 dark:text-slate-400">
  Generated by <code>build_site.py</code> from validated Pydantic checklists. Model provenance: %%MODEL%%.
</footer>
...
def main(*, combined: Path = COMBINED, out: Path = OUT) -> dict:
    data = load(combined)
    summaries = data["summaries"] if isinstance(data, dict) else data
    st = stats(summaries)
    st["model"] = model_provenance(summaries)
