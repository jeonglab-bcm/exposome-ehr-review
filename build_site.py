...
def stats(summaries: list[dict], model: str = "") -> dict:
    n = len(summaries)
...
<meta name="description" content="Searchable inventory of %%N%% pediatric exposome / EWAS manuscripts summarized with %%MODEL%%, %%EHR%% EHR-based, spanning %%YMIN%%-%%YMAX%%.">
...
<header class="border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/60 backdrop-blur sticky top-0 z-10">
  <div class="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between gap-4">
    <div>
      ...
      <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400">Summarized with %%MODEL%% · %%YMIN%%–%%YMAX%%</p>
    </div>
...
<footer class="border-t border-slate-200 dark:border-slate-800 py-6 text-center text-xs text-slate-500 dark:text-slate-400">
  Generated by <code>build_site.py</code> from the %%MODEL%% Pydantic checklists.
</footer>
...
def main() -> None:
    ...
    st = stats(summaries, model)
    recs = build_records(summaries)
    OUT.parent.mkdir(parents=True, exist_ok=True)
    page = (PAGE
            .replace("%%REPO%%", REPO)
            .replace("%%MODEL%%", html.escape(st["model"]))
            .replace("%%N%%", str(st["n"]))
